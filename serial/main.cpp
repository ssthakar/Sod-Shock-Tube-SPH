...
const int FLOPS_PER_VISCOSITY = 25;       // Additional FLOPs for artificial viscosity
...
int main(){
  ...
  metrics.stop();
  std::cout << "Simulation completed in " << metrics.getElapsedSeconds() << " seconds" << std::endl;
  std::cout << "Performance: " << metrics.getMflops() << " MFLOPS" << std::endl;
	return 0;
}
